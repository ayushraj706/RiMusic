...
import { 
  ...
  Lock, 
  MessageSquare,
  Mail,     // NAYA ADD KIYA (Agent Form ke liye)
  Loader2   // NAYA ADD KIYA (Loading spinner ke liye)
} from "lucide-react"; 
import { 
  motion, 
  AnimatePresence 
...
export default function LoginPage() {
  ...
  // 🔥 NAYA ADD KIYA: Admin ya Agent mode select karne ke liye states
  ...
  useEffect(() => {
    ...
    // 🔥 NAYA ADD KIYA: Check if Agent is already logged in via LocalStorage
    if (typeof window !== "undefined" && localStorage.getItem("agent_token")) {
      setIsAuthed(true);
    }
  ...
  // --- PURANE FIREBASE LOGIN FUNCTIONS (As it is) ---
  ...
  // 🔥 NAYA ADD KIYA: Agent Login Function
  const handleAgentLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // Agent login success! Token save karo local storage mein
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#FAFAFA] text-black font-sans relative overflow-hidden">
      <AnimatePresence mode="wait">
        ...
        {/* WATSAPP STYLE SPLASH SCREEN (PHASE 0 & 1) - PURANA AS IT IS */}
        {phase < 2 && (
          <motion.div
            key="splash-screen"
        ...
        {phase === 2 && !isAuthed && (
          ...
          >
            ...
            <motion.p 
              className="text-gray-500 font-light text-sm mb-6"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
            >
              Sign in to continue to your dashboard
            </motion.p>

            {/* 🔥 NAYA ADD KIYA: TOGGLE SWITCH FOR ADMIN vs AGENT */}
            <div className="flex bg-gray-100 p-1 rounded-xl mb-6">
              <button 
                onClick={() => setLoginMode("ADMIN")}
                className={`flex-1 text-[13px] py-2 rounded-lg font-semibold transition ${loginMode === "ADMIN" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
              >
                Business Owner
              </button>
              <button 
                onClick={() => setLoginMode("AGENT")}
                className={`flex-1 text-[13px] py-2 rounded-lg font-semibold transition ${loginMode === "AGENT" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
              >
                Support Agent
              </button>
            </div>
            ...
            >
              ...
              {/* CONDITION: Agar Admin mode hai toh purane buttons dikhao */}
              {loginMode === "ADMIN" ? (
                <>
                  <motion.button 
                ...
                /* 🔥 NAYA ADD KIYA: CONDITION: Agar Agent mode hai toh Email form dikhao */
                <form onSubmit={handleAgentLogin} className="space-y-3 text-left">
                  <div>
                    <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200 focus-within:border-[#25D366]">
                      <Mail className="w-5 h-5 text-gray-400" />
                      <input type="email" required placeholder="Agent Email" value={agentEmail} onChange={(e) => setAgentEmail(e.target.value)} className="bg-transparent flex-1 outline-none text-sm text-gray-900" />
                    </div>
                  </div>
                  <div>
                    <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200 focus-within:border-[#25D366]">
                      <Lock className="w-5 h-5 text-gray-400" />
                      <input type="password" required placeholder="Password" value={agentPassword} onChange={(e) => setAgentPassword(e.target.value)} className="bg-transparent flex-1 outline-none text-sm text-gray-900" />
                    </div>
                  </div>
                  <motion.button type="submit" disabled={isLoggingIn} variants={itemVariants} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="w-full bg-[#25D366] text-white py-3.5 rounded-xl font-bold hover:bg-[#1EBE5A] transition-colors shadow-sm flex items-center justify-center gap-2 mt-2 disabled:opacity-70">
                    {isLoggingIn ? <Loader2 className="w-5 h-5 animate-spin" /> : "Login to Workspace"}
                  </motion.button>
                </form>
              )}
