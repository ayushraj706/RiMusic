        ...
        <div className="relative max-w-[260px]">
          ...
          {msg.mediaType === "image" && msg.mediaUrl && (
            ...
            >
              ...
              {/* Inline action menu (Instagram-style bottom sheet) */}
              {menuOpen && (
                <div
...
export default function ChatDashboard() {
  ...
  // Chat open hone par history stack mein push karo
  // Android back press hone par activeContact null ho jaata hai
  useEffect(() => {
  ...
  const handleOpenChat = (contact: Contact) => {
    ...
    setSelectedMessages(new Set());
    setShowContactInfo(false);
    // Android default back button ke liye history push karo
    // Jab back press hoga, popstate fire hoga aur chat band hoga
  ...
  // ─── Send Media Message ───────────────────────────────────────────────────
  // Step 1: Upload file to WhatsApp media endpoint
  // Step 2: Get mediaId back
  // Step 3: Send media message via WhatsApp API
  // Step 4: Save to Firebase with mediaUrl (blob) + mediaId
  // Step 5: Show in chat as MediaMsgBubble
  const handleSendMedia = async (
    ...
    // Optimistic bubble — show immediately with local blob URL
    ...
    try {
      ...
      await set(newMsgRef, {
        ...
        status: "sent",
        mediaType: waType,
        mediaUrl: localUrl,   // blob URL — works for current session
        mediaName: file.name,
        mediaId,
      ...
      // ── 5. Replace optimistic bubble with real data ──
      setMessages((prev) => ({
        ...prev,
        [activeContact.id]: (prev[activeContact.id] || []).map((m) =>
          m.id === tempId
            ? { ...m, mediaId, status: "sent" }
            : m
        ),
      }));
    } catch (err: any) {
  ...
  return (
    <div className="flex h-[100dvh] w-full bg-white overflow-hidden">
      ...
      {/* ── Sidebar (always visible — fixed bottom nav on mobile) ── */}
      <Sidebar />
            ...
            <div id="mobile-chat-view" className="flex flex-col h-full z-10 w-full relative">
              ...
              <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-0.5">
                ...
                {messages[activeContact.id]?.map((msg) =>
                  // If message has media → use MediaMsgBubble, else normal ChatBubble
                  msg.mediaType ? (
              ...
              <ChatInput
                ...
                onSendLocation={(lat, lng) => {
                  // Optional: implement location message
                  console.log("Location:", lat, lng);
                }}
                onSendInteractive={(type) => {
                  // Optional: open interactive message builder
