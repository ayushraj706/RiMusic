...
// ─── Convert audio blob to ogg/opus (WhatsApp-compatible) ────────────────────
// WhatsApp accepts: audio/aac, audio/mp4, audio/mpeg, audio/amr, audio/ogg, audio/opus
// Browser records audio/webm — we rename to audio/ogg (same opus codec, different container label)
// Most modern browsers record opus inside webm; WhatsApp accepts ogg+opus.
// For best compatibility we send as audio/ogg with .ogg extension.
async function convertAudioToOgg(blob: Blob): Promise<File> {
  // If already ogg/opus/aac/mp4 audio, return as-is
  if (
    blob.type === "audio/ogg" ||
  ...
  // audio/webm → rename to audio/ogg (opus codec inside webm is same as ogg)
...
function MultiMediaBubble({ previews, isSending, onCancel, onSend, onAddMore }: MultiMediaBubbleProps) {
  ...
  return (
    <>
      ...
      {/* Stacked card preview (Instagram DM style) */}
      <div className="flex justify-end px-4 py-2">
        <div className="relative w-[260px]">
          {/* Stack layers behind (decorative) */}
          {items.length >= 3 && (
...
function AudioRecorder({ onStop, onCancel }: AudioRecorderProps) {
  ...
  useEffect(() => {
    navigator.mediaDevices.getUserMedia({ audio: true }).then((stream) => {
      // Try to record as audio/ogg directly if supported, else fallback to webm
...
export default function ChatInput({
  ...
  // Single input ref for multiple files
  ...
  // Multiple files picked → add to pending
  ...
  // Send all pending media one by one
  ...
  return (
    <div className="bg-[#f6f6f6]/80 backdrop-blur-lg border-t border-gray-200/50 z-20 relative">
      ...
      {/* ── Emoji Picker (iOS WhatsApp style, above input) ── */}
      {showEmojiPicker && (
        <div
          ref={emojiPickerRef}
          className="absolute bottom-full right-2 mb-2 z-40 shadow-2xl rounded-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-150"
        >
          <EmojiPicker
        ...
        {isRecording ? (
          ...
          <>
            ...
            <div className="flex-1 bg-white border border-gray-200 rounded-[22px] flex items-end px-3 py-1.5 min-h-[38px] shadow-sm">
              ...
              />
              {/* Emoji icon — iOS WhatsApp style, inside input pill */}
              <button
                onMouseDown={(e) => {
                ...
                className={`w-6 h-6 mb-1 ml-1 shrink-0 transition rounded-full ${
