...
import { auth, database } from "../lib/firebase"; // Path check kar lijiyega
...
export default function Sidebar() {
  ...
  // 2. Naye user ko Cloud mein "Match" (Save) karne ka function
  const handleMatchDevice = async () => {
    if (!user) return;
    
    // Yahan aage chalkar hum Meta API token lenge, abhi ke liye direct save kar rahe hain
    try {
      ...
      setIsMatched(true);
      alert("Success! Your device is now matched and saved to cloud.");
    } catch (error) {
