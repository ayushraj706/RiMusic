...
function buildPayload(form: FormState): CreateTemplatePayload {
  ...
  if (form.headerFormat !== "NONE") {
    ...
    if (form.headerFormat === HeaderFormat.TEXT) {
      ...
      // For media headers, use example if available
    ...
    <div className="relative mx-auto w-[270px] flex-shrink-0 mt-4">
      ...
      <div className="bg-[#1C1C1E] rounded-[45px] p-2.5 shadow-2xl shadow-gray-300/50 border-[3px] border-[#3A3A3C] ring-[1px] ring-gray-200">
        <div className="bg-[#EFEAE2] relative w-full h-[560px] rounded-[36px] overflow-hidden flex flex-col border-[4px] border-black">
          ...
          {/* Swipe Up/Down Controls for Preview */}
          <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-30 flex flex-col items-center gap-1">
            <button
              onClick={() => setIsExpanded(!isExpanded)}
              className="bg-black/60 backdrop-blur-sm text-white px-3 py-1.5 rounded-full text-[10px] font-bold flex items-center gap-1 hover:bg-black/80 transition"
            >
              {isExpanded ? <><ChevronDown className="w-3 h-3" /> Swipe Down</> : <><ChevronUp className="w-3 h-3" /> Swipe Up</>}
            </button>
          </div>

          {/* Expanded Preview Overlay */}
          {isExpanded && (
            <div className="absolute inset-0 bg-black/80 z-40 flex items-center justify-center p-4 animate-in fade-in duration-200">
              <div className="bg-white rounded-2xl w-full max-w-[240px] overflow-hidden shadow-2xl">
                <div className="bg-[#075E54] px-4 py-3 flex items-center justify-between">
                  <p className="text-white text-xs font-bold">Full Preview</p>
                  <button onClick={() => setIsExpanded(false)} className="text-white/80 hover:text-white">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                <div className="p-3 bg-[url('https://w0.peakpx.com/wallpaper/818/148/HD-wallpaper-whatsapp-background-cool-dark-green-light-pattern-texture.jpg')] bg-cover">
                  <div className="bg-white rounded-xl rounded-tl-none shadow-sm overflow-hidden">
                    ...
                    {hasButtons && (
                      <div className="border-t border-gray-200 bg-white">
                        {form.buttons.map((btn, i) => (
                          <div key={btn.id} className={`flex items-center justify-center gap-2 py-2.5 text-[14px] font-medium text-[#00A884] ${i < form.buttons.length - 1 ? "border-b border-gray-100" : ""}`}>
                            {btn.type === ButtonType.QUICK_REPLY && <CornerUpLeft className="w-4 h-4" strokeWidth={2.5} />}
                            {btn.type === ButtonType.URL && <ExternalLink className="w-4 h-4" strokeWidth={2.5} />}
                            {btn.type === ButtonType.PHONE_NUMBER && <Phone className="w-4 h-4" strokeWidth={2.5} />}
                            {btn.text || "Action Button"}
    ...
    if (initialData?.components) {
      ...
      initialData.components.forEach((comp: any) => {
        ...
        if (comp.type === "BUTTONS" && comp.buttons) {
          newForm.buttons = comp.buttons.map((btn: any, i: number) => ({
            id: uid(),
            type: btn.type,
            text: btn.text,
            url: btn.url,
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-gray-200 px-6 py-3.5 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            {isSubmitting ? "Saving..." : initialData ? "Update Template" : "Save & Submit to Meta"}
          </button>
        </div>
      </header>
...
export default function TemplateBuilderUI() {
  ...
  // 4. Edit Template (Delete old + Create new since Meta doesn't allow direct edits)
  ...
  return (
    <div className="min-h-screen bg-[#F4F7F6] text-gray-900 font-sans pb-20">
      ...
      <div className="px-6 pt-4 max-w-5xl mx-auto">
        <div className="flex flex-wrap gap-2">
          {["ALL", ...Object.values(TemplateCategory)].map((cat) => (
            <button
              key={cat}
            ...
            >
              {cat} {categoryCounts[cat] > 0 && <span className="ml-1 opacity-80">({categoryCounts[cat]})</span>}
            </button>
          ))}
        </div>
      </div>
        ...
        {errorMsg ? (
          ...
          <div className="text-center py-24 border-2 border-dashed border-gray-200 rounded-2xl bg-white shadow-sm">
            ...
            <p className="text-gray-500 font-medium mb-4">
              {filterCategory !== "ALL" ? `No ${filterCategory} templates found.` : "No templates found in this Meta account."}
            </p>
            <button 
